        ...
        *,
        ...
        if self.pack_fill_strategy in ("raw_best_fit", "raw_best_fit_knapsack"):
            if self.pack_max_duration is None:
                ...
                float(self.max_duration_per_rank)
                < float(self.max_cuts_per_rank) * float(self.pack_max_duration)
            ):
                raise ValueError(
                    "raw_best_fit packing requires --max-duration >= "
                    "--max-cuts * --concatenate-cuts-max-duration to keep "
                    "--max-duration as a strict per-rank upper bound. "
                    f"Got max_duration={self.max_duration_per_rank}, "
                    f"max_cuts={self.max_cuts_per_rank}, "
                    f"concatenate_cuts_max_duration={self.pack_max_duration} "
                    f"(product={float(self.max_cuts_per_rank) * float(self.pack_max_duration)})."
                )
