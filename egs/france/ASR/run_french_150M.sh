...
FEATURE_PREFETCH_BATCHES="${FEATURE_PREFETCH_BATCHES:-2}"
...
NODE_DATA_PRODUCER_PREFETCH_NEXT_EPOCH="${NODE_DATA_PRODUCER_PREFETCH_NEXT_EPOCH:-1}"
NODE_DATA_PRODUCER_CONTINUOUS_STREAM="${NODE_DATA_PRODUCER_CONTINUOUS_STREAM:-1}"
...
python ./zipformer/train.py \
  ...
  --feature-prefetch-batches "${FEATURE_PREFETCH_BATCHES}" \
  ...
  --node-data-producer-prefetch-next-epoch "${NODE_DATA_PRODUCER_PREFETCH_NEXT_EPOCH}" \
  --node-data-producer-continuous-stream "${NODE_DATA_PRODUCER_CONTINUOUS_STREAM}" \
