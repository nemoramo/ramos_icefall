...
## 9. Node Producer 可视化监控页（开端口访问）

如果你启用了 node producer（会生成 `node_producer_metrics.jsonl`），可以直接开一个轻量 HTTP 页面看 pipeline：

- 获取（Acquire / Fetch）：producer 实时产出速率
- 处理（Process / Pack）：有效产出速率和 backlog
- 消费（Consume）：每个 rank 的消费 step、queue depth、wait_ms
...
- `--history-limit`：服务端保留的最大记录数（默认 1200）
- `--history-points`：前端每次拉取的历史点数（默认 320）
- `--refresh-ms`：前端刷新周期（默认 1000ms）
- `--queue-size`：每 rank queue 容量（用于显示 fill ratio，默认 32）
